...
// const totalPrice = ({ price, discount, isInstallment, months }: ICalc): number => {
...
const posts = [
  ...
  const normalizeData = (unnormalizedData: PostItems[]): object => {
        const byId: {[key: string]: PostItems} = {}
        const allIds: string[] = unnormalizedData.map(item => {
            byId[item.id] = item
            return item.id
        })
         return {byId, allIds}
